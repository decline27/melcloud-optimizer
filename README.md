...
- **Fixed temperature control for ATW devices**: Implemented proper temperature control for Air-to-Water heat pumps using the correct effective flags value (`0x200000080`) based on the Home Assistant MELCloud integration
- **Real-time settings updates**: The app now reads the latest settings before each optimization run, ensuring that any changes to temperature limits, steps, or K-factor are immediately applied
